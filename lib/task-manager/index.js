...
async function _run(key) {
    try {
        ...
        const Fs = currentTask.Functions || []; // Multiple functions supported
        ...
        /* get load */
        // todo: make all loads async (couchdb isn't as of 2018-06-16)
        ...
        await asyncForEach(functions, async(f) => {
            item = await f.invoke.call(f);
            log.info("Task : " + key + " " + ": Function executed");
        });
