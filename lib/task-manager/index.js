...
function _run(key) {
    return new Promise(function(finishedRunFn) {
        ...
        var batch = null;
        return (function doEtlTask(self) {
                return new Promise(function(doNextBatch, allBatchesDone) {
                    console.log('starting doETLTask')
                    var payload = [];
                    ...
                    if (!batch.done) {
                        ...
                        return (function getBatchItem(i) {
                                return new Promise(function(transformNextItem, batchTransformed) {
                                    console.log('starting getBatchItem')
                                    if (i < batch.length) {
                                        var item = batch[i];

                                        /* Do all transformations on each item */
                                        return (function transformItem(j) {
                                                return new Promise(function(doNextTransformation, allTransformationsDone) {
                                                    console.log('starting transformItem')
                                                    if (item && j < transformations.length) {
                                                        const t = transformations[j];
                                                        return t.transform.call(t, item)
                                                            .then(function(transformedItem) {
                                                                item = transformedItem;
                                                                j++;
                                                                console.log(j)
                                                                return doNextTransformation([transformItem, j]);
                                                            })
                                                        ...
                                                        return allTransformationsDone(function() {
                                                            console.log('transforming next item')
                                                            if (item !== {} && item) payload.push(item);
                                                            i++;
                                                            return transformNextItem([getBatchItem, i]);
                                                        });
                                                    };
                                                });
                                            })(0).then(function(args) {
                                                const transformItem = args[0];
                                                const j = args[1];
                                                console.log(j)
                                                return transformItem(j);
                                            })
                                            .catch(function(error) {
                        ...
                        allBatchesDone(function() {
                            ...
                            finishedRunFn(endMsg);
                        });
                    };
                });
            })(this).then(function(args) {
                const doEtlTask = args[0];
                const self = args[1];
                doEtlTask(self);
            })
            .catch(function(error) {
