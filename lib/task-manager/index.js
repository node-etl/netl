...
async function _run(key) {
    try {
        ...
        /* Do ETL if required */
        if (etl) await _runEtl.call(this, currentTask, etl);
        const E = currentTask.Extraction || null; // Only single extraction (or none) supported
        const batchSize = (E) ? E.batchSize : null;
        const Ts = currentTask.Transformations || null; // Multiple transformations (or none) supported
        const L = currentTask.Load || null; // Only single load (or none) supported
        const Fs = currentTask.Functions || null; // Multiple functions (or none) supported
        ...
        var batches;
        if (E) {
            extraction = await this.extractions[E.Name].call(E);
        ...
        var transformations;
        if (Ts)
            transformations = await Ts.reduce(async(accumulator, t) => {
        ...
        /* Setup load */
        var load;
        if (L) load = await this.loads[L.Name].call(L);

        /* Setup functions */
        var functions;
        if (Fs)
            functions = await Fs.reduce(async(accumulator, f) => {
                const arr = await accumulator;
