...
async function _runEtl(key, currentTask, etl) {
    if (!(etl.Extraction && etl.Transformations && etl.Load)) throw new Error(`Task : ${key} : ETL specified but not configured correctly`);

    // Get directory of extraction path
    etl.Extraction.path = etl.Extraction.path.replace(/\\/g, "/");

    // Check directory exists; test absolute, then relative directory path
    const eDirPath = etl.Extraction.path.substring(0, etl.Extraction.path.lastIndexOf("/") + 1);
    if (!fs.existsSync(path.normalize((eDirPath)))) {
        if (!fs.existsSync(path.join(__dirname, eDirPath))) throw new Error("Extraction directory path doesn't exist");
    };

    // Get all paths in directory
    const extractionPaths = await new Promise((resolve, reject) => {
        glob(etl.Extraction.path, {}, (err, files) => {
            if (err) reject(err);
            else resolve(files);
        });
    });

    // Run the ETL task for each path
    await asyncForEach(extractionPaths, async(p) => {
        const etlClone = deepClone(etl);
        const E = etlClone.Extraction || null; // Only single extraction supported
        const Ts = etlClone.Transformations || null; // Multiple transformations supported
        const L = etlClone.Load || null; // Only single load supported
        E.path = p;
...
async function _runFunctions(key, currentTask, Fs) {
    ...
    await asyncForEach(functions, async(f) => {
        ...
        log.info(`Task : ${key} : Function execution complete`);
    });
};
...
async function _run(key) {
    try {
        ...
        return `Task : ${key} complete with error: ${(error.stack) ? error.stack : error}`;
    };
};
